class Queen extends Piece {
	...
	getmoves(game){
		...
		for (let h = 1; h < 8; h++){
			if (this.rank+h < 8){
				moves.push([7-(this.rank+h),this.file]);
				if (game[this.rank+h][this.file] !== ""){
					//console.log("test: " + game[this.rank+h][this.file+h].rank);
					break;
				}
		...
		for (let j = -1; j > -8; j--){
			if (this.rank+j > -1){
				moves.push([7-(this.rank+j),this.file]);
				if (game[this.rank+j][this.file] !== ""){
					//console.log("test: " + game[this.rank+e][this.file+e].rank);
					break;
				}	
			}
		...
		for (let k = 1; k < 8; k++){
			if (this.file+k < 8){
				moves.push([7-this.rank,this.file+k]);
				if (game[this.rank][this.file+k] !== ""){
					//console.log("test: " + game[this.rank+f][this.file-f].rank);
					break;
				}	
			}
		...
		for (let l = 1; l < 8; l++){
			if (this.file-l > -1){
				moves.push([7-this.rank,this.file-l]);
				if (game[this.rank][this.file-l] !== ""){
					//console.log("test: " + game[this.rank+f][this.file-f].rank);
					break;
				}	
			}
